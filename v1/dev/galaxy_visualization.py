...
mpl.rcParams.update({
    ...
    "xtick.direction": "in",
    "ytick.direction": "in",
    "text.usetex": True  # if LaTeX installed
})
...
'''
lines=["H1_6562.80A", "H1_4861.35A", "O1_1304.86A","O1_6300.30A","O2_3728.80A","O2_3726.10A",
...
wavelengths=[6562.80, 4861.35, 1304.86, 6300.30, 3728.80, 3726.10, 
...
class VisualizationManager:
    ...
    # TODO initialise with lims dict
    ...
    #------------------------------------------------------------------
    # Simulation Information and Helpers
    #------------------------------------------------------------------
    ...
    def render_image(self,
                     ...
                     lims: Optional[Tuple[float, float]] = None,
                     log: bool = True,
                     save_fits: bool = True) -> None:
        """
        Render publication-quality 2D image and save to disk.
        ...
        if save_fits:
            self.write_fits_image(
                image,
                f"{self.output_dir}/{filename}.fits",
                field=str(field),
                width=width,
                center=center,
                redshift=redshift
    ...
    def projection_plot(self,
        ...
        """
        Generate projection plot and return image array.

        Parameters
        ----------
        ds : yt.Dataset
            Loaded dataset.
        sp : yt.data_objects.data_containers.YTSphere
            Sphere defining region.
        field : tuple
            Field to project.
        width : tuple
            Projection width.
        ...
        self.render_image(image,
                          ...
                          center,
                          ds.current_redshift,
                          title=f"{field[1]} Projection",
                          filename=f"proj_{field[1]}",
                          lims=lims,
                          log=log)
    ...
    def slice_plot(self,
               ...
               log: bool = True) -> np.ndarray:
        """
        Generate slice plot and return image array.
        ...
        center : ndarray
            Slice center.
        lims : tuple, optional
            (vmin, vmax) normalization limits.
        log : bool
        ...
        self.render_image(image,
                          ...
                          center,
                          ds.current_redshift,
                          title=f"{field[1]} Slice",
                          filename=f"slice_{field[1]}",
                          lims=lims,
                          log=log)
    ...
    def star_gas_overlay(self,
                     ds,
                     sp,
                     gas_field: Tuple[str, str],
                     width: Tuple[float, str],
                     lims: Optional[Tuple[float, float]] = None,
                     log: bool = True,
                     filename: str = "star_gas_overlay") -> np.ndarray:
        """
        Generate gas projection with star particle overlay.

        Parameters
        ----------
        ds : yt.Dataset
        sp : yt data container
            Region defining projection.
        gas_field : tuple
            Gas field to project.
        width : tuple
            Projection width.
        lims : tuple, optional
            (vmin, vmax) normalization limits.
        ...
        -------
        image : ndarray
            Gas projection image (without star overlay).
        """
        ...
        # Gas projection
        proj = yt.ProjectionPlot(ds, "z", gas_field,
                                 center=center,
                                 width=width,
                                 data_source=sp)

        frb = proj.data_source.to_frb(width, self.buff_size)
        image = np.array(frb[gas_field]).astype(np.float32)
        ...
        x = sp["star", "particle_position_x"].to(width[1])
        y = sp["star", "particle_position_y"].to(width[1])

        # Convert to pixel coordinates
        box_half = width[0] / 2
        x_pix = (x - center[0]) / width[0] * self.buff_size + self.buff_size / 2
        y_pix = (y - center[1]) / width[0] * self.buff_size + self.buff_size / 2
        ...
        cbar = fig.colorbar(im, ax=ax)
        cbar.set_label(gas_field[1])

        ax.set_title("Gas + Star Overlay")
        ax.set_xlabel(f"X [{width[1]}]")
        ax.set_ylabel(f"Y [{width[1]}]")
    ...
    def plot_wrapper(self,
                 ...
                 width,
                 center,
                 plot_type: str = "projection",
                 weight_field=None,
                 lims_dict: Optional[Dict] = None,
                 log: bool = True) -> Dict[str, np.ndarray]:
        """
        High-level driver for projection or slice plots over multiple fields.

        This function loops over a list of fields and generates
        either projection or slice plots for each field. Figures
        are rendered using the central backend and saved as
        PDF, PNG, and FITS files.
        ...
        sp : yt data container
            Region of interest (used for projections).
        field_list : tuple or list of tuple
            Field or list of fields, e.g.
            [('gas','density'), ('gas','temperature')].
        width : tuple
            Plot width in the form (value, unit).
        center : ndarray
            Plot center coordinates.
        plot_type : str
            'projection' or 'slice'.
        weight_field : tuple, optional
            Weight field for projections.
        lims_dict : dict, optional
            Dictionary mapping field -> (vmin, vmax).
            Enables consistent normalization across time slices.
        log : bool
        ...
        # Allow single field input
        if isinstance(field_list, tuple):
        ...
        for field in field_list:
            ...
            # Retrieve limits for this field if provided
            lims = None
            if lims_dict is not None:
                lims = lims_dict.get(field)
    ...
    def save_sim_info(self, ds=None):
        ...
        Parameters:
        ds: RAMSES data loaded into yt (optionally specify; otherwise use
            object-stored ds)
        '''
    ...
    def compute_line_luminosity(self,
                            sp,
                            emissivity_field: Tuple[str, str]) -> float:
        """
        Compute total line luminosity within a data container.

        Parameters
        ----------
        sp : yt data container
            Data source (sphere or region).
        emissivity_field : tuple
            Emissivity field in units of erg/s/cm^3.

        Returns
        -------
        luminosity : float
            Total luminosity in erg/s.
        """

        emissivity = sp[emissivity_field]  # erg/s/cm^3
        cell_volumes = sp["gas", "cell_volume"].to("cm**3")

        luminosity = (emissivity * cell_volumes).sum().to("erg/s")

        return luminosity.value


    def compute_multiple_lines(self,
                           sp,
                           line_fields: List[Tuple[str, str]]) -> Dict[str, float]:
        """
        Compute luminosities for multiple emission lines.

        Parameters
        ----------
        sp : yt data container
        line_fields : list of tuple
            Emissivity fields.

        Returns
        -------
        luminosities : dict
            Dictionary {line_name: luminosity}.
        """

        luminosities = {}

        for field in line_fields:
            L = self.compute_line_luminosity(sp, field)
            luminosities[field[1]] = L
    ...
    def luminosity_to_flux(self,
        ...
        -------
        flux : float
            Observed flux in erg/s/cm^2.
        """

        d_l = Planck18.luminosity_distance(redshift).to("cm")
        flux = luminosity / (4 * np.pi * d_l.value**2)
    ...
    def proj_plot(self, sp, width, center, field, weight_field, ds=None):
        ...
        weight_field (tuple, str): field to weight project (or None if 
            unweighted)
        ds: loaded RAMSES-RT data set (optionally specify; otherwise use
            object-stored ds)
    ...
    def slc_plot(self, ds, width, center, field):
        ...
        center (List, float): center (array of 3 values) in code units
        field (tuple, str): field to project, e.g. ('gas', 'temperature')
        ds: load RAMSES-RT data set (optionally specify; otherwise use
            object-stored ds)
    ...
    def convert_to_plt(self, yt_plot, plot_type, field, width, 
        ...
        if lims == None:
            ...
            vmin = np.min(p_img[p_img > 0])
            vmax = np.max(p_img)
            if vmin == vmax:
                vmax = vmin * 10
            dens_norm = LogNorm(vmin=vmin, vmax=vmax)
        else:
    ...
    def plot_wrapper(self, sp, width, center, field_list,
                     weight_field_list, title_list, ds=None, proj=True, 
                     slc=True, lims_dict=None):
        '''
        ...
        ds: loaded RAMSES-RT data set (optionally specify; otherwise use
            object-stored ds)
        proj (bool): flag to plot projection
        slc (bool): flag to plot slice
    ...
    def phase_with_profiles(self, phase_profile,
        ...
        if linear:
            ...
            ax1.scatter(
                ...
                peak_x, avg_z_vals_x[np.argmin(np.abs(x_vals - peak_x))],
                color='red', s=50)
        else:
            ...
            ax1.scatter(
                ...
                peak_x, np.log10(avg_z_vals_x[np.argmin(np.abs(x_vals - peak_x))]),
                color='red', s=50)
        ...
        if linear:
            ...
            ax2.scatter(
                ...
                avg_z_vals_y[np.argmin(np.abs(y_vals - peak_y))], peak_y,
                color='red', s=50)
            ...
            ax2.scatter(
                ...
                np.log10(avg_z_vals_y[np.argmin(np.abs(y_vals - peak_y))]), peak_y,
                color='red', s=50)
